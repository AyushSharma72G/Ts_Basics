...
const User: UserInterface = {
  ...
  getAge(): number {
    if (this.age === undefined) {
      // the age can be undefined as it is optional field inn the interface
...
try {
  ...
  if (error instanceof Error) {
    // check if the error is the one which we throw from the getAge function
...
// any type
let value1: any = "values";
value1 = 10;
value1.toUpperCase(); // no error with the type any ts behave just like normal js

// unknown
let value: unknown = "hello";
value = 10; // allowed  unknown is used in api response(the backend might break, user input user can give unprectiable inputs )
if (typeof value === "string") {
  // can assign anything to unknown, but you cannot use it without checking
